...
<script type="text/javascript">
    ...
    function deleteBatch() {
            //取得所有被选中删除商品的pid，根据其长度判断是否有选中的商品
            ...
            if(cks.length==0){
                ...
                var str="";
                var pid="";
                // 有选中的商品，则取其id的值，进行字符串拼接
                if(confirm("您确定删除"+cks.length+"条商品吗？")){
                    //拼接ID
                    $.each(cks,function () {
                        pid = $(this).val(); //获取当前对象的id
                        //进行非空判断，避免出错
                        if(pid!=null)
                            str += pid+",";  //1，4，5，
                    });
...
<script type="text/javascript">
    function ajaxsplit(page) {
        //异步ajax分页请求
        //向服务器发出ajax请求，请求page页中所有数据，在当前页面上局部刷新显示
        $.ajax({
    ...
    function condition(){
        ...
        $.ajax({
            uri:"${pageContext.request.contextPath}/prod/condition.action",
            data:{"pname":pname,
                "typeid":typeid,
                "lprice":lprice,
                "hprice":hprice},
